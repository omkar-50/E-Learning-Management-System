...
if(isset($_SESSION['is_admin_login'])){
    ...
    echo "<script>document.getElementsByTagName('title')[0].innerHTML='Add Course';</script>";
}
else{
...
if(isset($_REQUEST['courseSubmitBtn'])){
    ...
    (!is_numeric($_REQUEST['course_original_price'])))
    {
        ...
        if($conn->query($sql) == TRUE){
            $msg = '<div class="alert alert-success col-sm-6 mt-3">Course Added Successfully</div>';
        }
        else{
            $msg = '<div class="alert alert-danger col-sm-6 mt-3">Unable to Add Course</div>';
        }
